...
Environment:
  VERSION   Release tag or semver (e.g. v0.1.0 or 0.1.0). Default: latest.
...
verify_checksum() {
  ...
  if [ ! -f "$sums" ]; then
    echo "install.sh: checksums.txt not found; skipping sha256 verification"
    return 0
  fi

  expected="$(awk -v name="$name" '$2 == name { print $1; exit }' "$sums")"
  if [ -z "$expected" ]; then
    echo "install.sh: no checksum entry for ${name}; skipping verification" >&2
    return 0
  fi

  actual="$(sha256_file "$archive")"
  if [ -z "$actual" ]; then
    echo "install.sh: no sha256sum/shasum on PATH; skipping verification" >&2
    return 0
  fi
